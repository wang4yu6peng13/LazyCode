...
def deal_table_line(line):
    if "<table" in line or "<const_table" in line:
        ...
        if m:
            name = m.group(1)
            line = comments + '%s<%stable export="server|client|all" name="%s" from="mzm.gsp.%s.%s.%s"%s>\n' % (
                SPACE4*2, "const_" if "const_" in line else "", name, NAMESPACE1, NAMESPACE2, name, "" if "const_" in line else ' pml_type="fixed|dynamic"')
    return line


def deal_enum_line(line):
    if "<enum" in line:
        line = line.replace("enum", 'enum export="all|server|client"')
    return line
